...
CLASS lhc_SalesOrderItem IMPLEMENTATION.
  METHOD update.
    DATA: ls_sales_itm TYPE zcit_vicky_itm.
    LOOP AT entities INTO DATA(ls_entities).
      ls_sales_itm = CORRESPONDING #( ls_entities MAPPING FROM ENTITY ).
      IF ls_sales_itm-salesdocument IS NOT INITIAL.
        SELECT FROM zcit_vicky_itm FIELDS * WHERE salesdocument = @ls_sales_itm-salesdocument AND salesitemnumber = @ls_sales_itm-salesitemnumber
        ...
        IF sy-subrc EQ 0.
          ...
          IF lv_created EQ abap_true.
            ...
            APPEND VALUE #( %key = ls_entities-%key
                            %msg = new_message( id = 'ZCIT_VICKY_MSG' number = '001' v1 = 'Sales Item Updation Successful'
                                                severity = if_abap_behv_message=>severity-success ) ) TO reported-salesorderitm.
          ENDIF.
        ELSE.
          APPEND VALUE #( salesdocument = ls_sales_itm-salesdocument
          ...
          APPEND VALUE #( salesdocument = ls_sales_itm-salesdocument
                          ...
                          %msg = new_message( id = 'ZCIT_VICKY_MSG' number = '001' v1 = 'Duplicate Sales Order'
  ...
  METHOD delete.
    ...
    LOOP AT keys INTO DATA(ls_key).
      ...
      APPEND VALUE #( salesdocument = ls_key-salesdocument
                      ...
                      %msg = new_message( id = 'ZCIT_VICKY_MSG' number = '001' v1 = 'Sales Item Deletion Successful'
                                          severity = if_abap_behv_message=>severity-success ) ) TO reported-salesorderitm.
    ENDLOOP.
  ENDMETHOD.
